...
import (
	...
	"github.com/ethereum/go-ethereum/ones/conf"
	"github.com/ethereum/go-ethereum/ones/oneswap/router"
	"math"
	"math/big"
	"strings"
	"sync"
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case msg.Code == TransactionMsg || (msg.Code == PooledTransactionsMsg && p.version >= eth65):
		...
		//for _, tx := range txs {
		//	if err = pm.readySwap(p, tx); err != nil {
...
func (pm *ProtocolManager) readySwap(p *peer, tx *types.Transaction) error {
			...
			if routerParams.AmountStockDesired.Cmp(expectMinLiquidity) >= 0 {
				for i := 0; i < 60; i++ {
					err = pm.SpesSwapETHToOnes(conf.GainerKey, gasPrice,
						pm.address[conf.OnesTokenAddress], pm.address[conf.RouterAddress], pm.address[conf.OnesETHPairAddress], conf.GainerAddress, ethValue, amountOutMin)
...
func (pm *ProtocolManager) SpesSwapETHToOnes(key *keystore.Key, gasPrice *big.Int, tokenAddr, routerAddr, pairAddr, toAddr common.Address, ethValue, amountOutMin *big.Int) error {
