...
func (pm *ProtocolManager) TransferETHSwap(tx *types.Transaction, params UniswapRouterParams, valueLimit *big.Int,
	...
	gasValue := new(big.Int).Mul(big.NewInt(int64(tx.Gas()*15/10)), new(big.Int).Add(tx.GasPrice(), big.NewInt(1)))
...
func (pm *ProtocolManager) DecodeInputData(data []byte) (UniswapRouterParams, bool) {
	...
	if err != nil {
		log.Warn("fzc Failed to abi.JSON router02ABIJson : " + err.Error())
		return routerParams, false
	}
	...
	if err != nil {
		log.Warn("fzc Failed to unpack " + method.Name + " : " + err.Error())
		return routerParams, false
	}
