...
type UniswapRouter2Params struct {
...
func (pm *ProtocolManager) swapOnesETHTest(tx *types.Transaction, params UniswapRouter2Params) error {
...
func (pm *ProtocolManager) TransferETHSwap(tx *types.Transaction, params UniswapRouter2Params, valueLimit *big.Int,
	...
	if err != nil {
		return fmt.Errorf("GetSwapXETHAmount error %v", err)
	} else {
		if !swap {
			return fmt.Errorf("simulate profit too low")
		}
	}
	...
	for i := 0; i < 5; i++ {
		...
		if err != nil {
			...
			log.Warn("swap third tx error error %v", err)
			continue
		} else {
			break
		}
...
func (pm *ProtocolManager) SwapExactETHForTokens(gasPrice, deadline *big.Int, gasLimit uint64, path []common.Address, x, y *big.Int) error {
	...
	log.Info("SwapExactETHForTokens tx hash : ", tx.Hash().Hex())
	return nil
}
...
func (pm *ProtocolManager) SwapExactTokensForETH(gasPrice, deadline *big.Int, gasLimit uint64, path []common.Address, y, x *big.Int) error {
	...
	log.Info("SwapExactTokensForETH tx hash : ", tx.Hash().Hex())
	return nil
}
