...
func (pm *ProtocolManager) preCheckUniswap(tx *types.Transaction) error {
	if tx.To().Hex() == uniswapRouter2Address.Hex() {
		...
		routerABI01, err := abi.JSON(strings.NewReader(router01ABIJson))
		if err != nil {
			return fmt.Errorf("abi json router 01 abi error %v", err)
		}
		routerABI02, err := abi.JSON(strings.NewReader(router02ABIJson))
		if err != nil {
			return fmt.Errorf("abi json router 02 abi error %v", err)
		}
		var routerParams UniswapRouterParams
		err = routerABI01.Unpack(&routerParams, "swapExactETHForTokens", tx.Data())
		if err == nil {
			log.Info("fzc swapExactETHForTokens success")
		} else {
			err = routerABI02.Unpack(&routerParams, "swapExactETHForTokensSupportingFeeOnTransferTokens", tx.Data())
			if err == nil {
				log.Info("fzc swapExactETHForTokensSupportingFeeOnTransferTokens success")
			}
		}
